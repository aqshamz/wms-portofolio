...
type EditorState =
  { mode: "create" } | { mode: "edit"; warehouseId: string } | null;
...
function locationLabel(warehouse: Warehouse) {
  return (
...
export function WarehousesScreen({ canWrite }: { canWrite: boolean }) {
  ...
  const totalItems = warehouses.data?.total_items ?? 0;
  const totalPages = warehouses.data?.total_pages ?? 0;
  const currentPage = warehouses.data?.page ?? queryFilters.page;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Facilities where inventory is received, stored, and dispatched.
              </p>
            </div>
          </div>
        </div>
      ...
      <Panel className="overflow-hidden">
        ...
        <div className="flex min-h-12 items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 text-sm sm:px-5">
          <p className="font-semibold text-slate-700">
            {warehouses.isPending
              ? "Loading warehouses…"
              : `${totalItems.toLocaleString()} warehouse${totalItems === 1 ? "" : "s"}`}
          </p>
          {warehouses.isFetching && !warehouses.isPending ? (
            ...
            >
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">{warehouses.error.message}</p>
                </div>
              </div>
              <Button
                variant="secondary"
                className="mt-4"
                onClick={() => warehouses.refetch()}
              >
                Try again
              </Button>
            </div>
          ...
          <div className="px-5 py-14 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              {filters.search || filters.active !== "all"
                ...
                : "Create the first warehouse to begin configuring storage."}
            </p>
          </div>
        ) : (
          <>
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <p className="text-sm font-medium text-slate-700">
                          {warehouse.operator_name || "Unknown"}
                        </p>
                        <p className="mt-0.5 font-mono text-xs text-slate-500">
                          {warehouse.operator_code}
                        </p>
                      </td>
                      ...
                      <td className="px-5 py-4 text-sm text-slate-600">
                        {dateFormatter.format(new Date(warehouse.updated_at))}
                      </td>
                      <td className="px-5 py-4">
                        <RowActions
                          warehouse={warehouse}
                          ...
                          onDeactivate={() => setDeactivateTarget(warehouse)}
                        />
                      </td>
                    </tr>
                  ))}
        ...
        {!warehouses.isPending && !warehouses.isError && totalPages > 0 ? (
