...
import { balanceKeys, listBalances } from "./inventory-api";
import { InventoryBalanceDetailDialog } from "./inventory-detail-dialog";
import type { Balance, BalanceFilters } from "./inventory-types";
...
export function InventoryBalancesScreen({ timezone }: { timezone: string }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Current on-hand, reserved and available stock by inventory
              dimension.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4 text-sm text-cyan-950 sm:p-5">
        Balances are read-only stock truth. Receive, putaway, picking, transfer,
        adjustment and other controlled workflows create the changes shown here.
      </section>

      <Panel className="overflow-hidden">
        <form
          className="grid gap-3 border-b border-slate-200 p-4 sm:p-5 md:grid-cols-2 xl:grid-cols-[1fr_1fr_auto_2fr_auto]"
          onSubmit={(event) => {
        ...
        >
          <Select
            ariaLabel="Filter balances by warehouse"
          ...
          />
          <Select
            ariaLabel="Filter balances by owner"
            value={filters.owner}
          ...
          <label className="flex h-11 items-center gap-2 rounded-xl border border-slate-300 bg-white px-3 text-sm text-slate-700">
            <input
              ...
              onChange={(event) =>
                void setFilters({ includeZero: event.target.checked, page: 1 })
              }
            />
            Include zero
          </label>
          <div className="relative">
            ...
            <Input
              className="mt-0 pl-9"
              aria-label="Search inventory balances"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3 text-sm sm:px-5">
          <p className="text-slate-600">
            {owner && warehouse
              ? `${balances.data?.total_items ?? 0} balances`
        ...
        {error ? (
          ...
          <EmptyState
            title="Select a warehouse and served owner"
            text="Only balances within your access scope will be requested."
          />
        ) : balances.isPending ? (
          <div className="flex min-h-56 items-center justify-center gap-2 text-sm text-slate-600">
            <LoaderCircle className="size-5 animate-spin" /> Loading balances…
          </div>
        ) : !balances.data?.items.length ? (
          <EmptyState
            title="No inventory balances found"
            text="Try another search or include zero balances."
          />
        ) : (
          <BalanceRows
          ...
          <tbody>
            {rows.map((balance) => (
              ...
              >
                ...
                <td className="px-5 py-4">
                  {balance.lot_number ??
                    (balance.handling_unit_id ? "Handling unit" : "—")}
                </td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  {updatedAt(balance.updated_at, timezone)}
                </td>
                <td className="px-5 py-4">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => onSelect(balance.balance_id)}
                  >
          ...
          <article key={balance.balance_id} className="p-4">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <dd className="mt-1">{balance.location_code}</dd>
              </div>
              <div>
