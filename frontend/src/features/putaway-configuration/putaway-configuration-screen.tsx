...
import {
  deactivatePutawayStrategy,
  deactivatePutawayStrategyRule,
  listPutawayStrategies,
  listPutawayStrategyRules,
  putawayConfigurationKeys,
} from "@/features/putaway-configuration/putaway-configuration-api";
import {
  PutawayConfigurationDialog,
  type PutawayEditor,
} from "@/features/putaway-configuration/putaway-configuration-dialog";
import type {
  PutawayStrategy,
  PutawayStrategyRule,
...
type DeactivateTarget =
  ...
  | { kind: "rule"; strategy: PutawayStrategy; item: PutawayStrategyRule };
        ...
        >
          ...
          <Dialog.Description className="mt-2 text-sm leading-6 text-slate-600">
            {target?.kind === "strategy"
              ? "Its ordered destination rules remain recorded, but new putaway work will no longer select this strategy."
              : "This condition will no longer participate in destination evaluation."}
          </Dialog.Description>
          {error ? (
            <p
              role="alert"
...
export function PutawayConfigurationScreen({
  canWrite,
}: {
  canWrite: boolean;
}) {
  ...
  const [editor, setEditor] = useState<PutawayEditor>();
  ...
  const strategies = useQuery({
    queryKey: putawayConfigurationKeys.strategies(strategyFilters),
    queryFn: () => listPutawayStrategies(strategyFilters),
    placeholderData: keepPreviousData,
  });
  const selectedStrategy = useMemo(() => {
    const items = strategies.data?.items ?? [];
    return (
      items.find((item) => item.putaway_strategy_id === filters.strategy) ??
      items[0]
    );
  }, [filters.strategy, strategies.data?.items]);
  const rules = useQuery({
    queryKey: putawayConfigurationKeys.rules(
      selectedStrategy?.putaway_strategy_id ?? "none",
      allRecords,
    ),
    queryFn: () =>
      listPutawayStrategyRules(
        selectedStrategy!.putaway_strategy_id,
        allRecords,
      ),
  ...
  const categories = useQuery({
    queryKey: putawayConfigurationKeys.categories(
      selectedStrategy?.owner_id ?? "global",
    ),
    queryFn: () =>
      listItemCategories({
        ownerId: selectedStrategy!.owner_id!,
        search: "",
        active: "all",
        page: 1,
        pageSize: 100,
      }),
    enabled: Boolean(selectedStrategy?.owner_id),
  });
  ...
  const zones = useQuery({
    ...
    enabled: Boolean(selectedStrategy?.warehouse_id),
  });
  const deactivate = useMutation<
    PutawayStrategy | PutawayStrategyRule,
    Error,
    DeactivateTarget
  >({
    mutationFn: (target) =>
      target.kind === "strategy"
        ? deactivatePutawayStrategy(target.item.putaway_strategy_id)
        : deactivatePutawayStrategyRule(
    ...
    onSuccess: async () => {
      ...
      toast.success("Putaway configuration deactivated.");
  ...
  const categoryById = new Map(
    (categories.data?.items ?? []).map((item) => [item.category_id, item]),
  );
  const locationTypeById = new Map(
    (locationTypes.data ?? []).map((item) => [item.location_type_id, item]),
  );
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Guide received inventory toward eligible storage destinations.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[19rem_minmax(0,1fr)]">
        <Panel className="h-fit overflow-hidden">
          <div className="border-b border-slate-200 p-4">
            <h2 className="font-bold text-slate-950">Putaway strategies</h2>
          ...
          >
            ...
            <Select
              ...
              onValueChange={(active) =>
                void setFilters({ active, strategy: "" })
              }
            />
          </form>
          {strategies.isPending ? (
            ...
            <div className="max-h-[34rem] space-y-1 overflow-y-auto p-2">
              {strategies.data.items.map((item) => {
                const active =
                  selectedStrategy?.putaway_strategy_id ===
                  item.putaway_strategy_id;
                return (
                  <button
                    key={item.putaway_strategy_id}
                    type="button"
                    onClick={() =>
                      void setFilters({ strategy: item.putaway_strategy_id })
                    }
                    className={cn(
          ...
          <Panel className="grid min-h-80 place-items-center p-8 text-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Its ordered destination rules will appear here.
              </p>
            </div>
          </Panel>
        ) : (
          <div className="min-w-0 space-y-4">
                ...
                {canWrite ? (
                  <div className="flex shrink-0 gap-1">
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setEditor({ kind: "strategy", item: selectedStrategy })
                      }
                    >
            ...
            <Panel className="overflow-hidden">
              <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
                <div>
                  <h2 className="font-bold text-slate-950">
                    Ordered destination rules
                  </h2>
                  <p className="mt-1 text-sm text-slate-500">
                    Specific conditions should precede broad catch-all rules.
                  </p>
                </div>
                {canWrite ? (
                  <Button
              ...
              {rules.isPending ? (
                ...
                <ErrorState
                  title="Putaway rules could not be loaded"
                  error={rules.error}
                ...
                <div className="px-5 py-14 text-center">
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Create the first ordered destination rule.
                  </p>
                </div>
              ) : (
                      ...
                      <RuleCard
                        ...
                        locationType={
                          item.location_type_id
                            ? locationTypeById.get(item.location_type_id)
                            : undefined
                        }
                        zone={
                          item.zone_id ? zoneById.get(item.zone_id) : undefined
                        }
                        canWrite={canWrite}
                        onEdit={() =>
                          setEditor({
...
function RuleCard({
  ...
  locationType?: { name: string; code: string };
  ...
  const conditions = [
    category ? `Category: ${category.name} (${category.code})` : "Any category",
    locationType
      ? `Location: ${locationType.name} (${locationType.code})`
      : "Any location type",
    zone ? `Zone: ${zone.name} (${zone.code})` : "Any zone",
    item.minimum_empty_percent
      ? `At least ${item.minimum_empty_percent}% empty`
