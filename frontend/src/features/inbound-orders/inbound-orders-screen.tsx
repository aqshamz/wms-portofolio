...
import { InboundOrderDetailDialog } from "@/features/inbound-orders/inbound-order-detail-dialog";
import { InboundOrderFormDialog } from "@/features/inbound-orders/inbound-order-form-dialog";
import type { InboundOrderStatus } from "@/features/inbound-orders/inbound-order-types";
import {
  listWarehouseOwners,
  listWarehouses,
  warehouseKeys,
...
export function InboundOrdersScreen({
  canPlan,
  canApprove,
  canCancel,
}: {
  ...
  const orders = useQuery({
    queryKey: inboundOrderKeys.list(queryFilters),
    queryFn: () => listInboundOrders(queryFilters),
    ...
    <div className="space-y-6">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">
                Schedule approved Purchase Order quantities for receiving.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
        ...
        <p className="mt-3 text-sm text-slate-600">
          Drafts remain editable. Releasing an order freezes its plan and makes
          it eligible for receipt creation.
        </p>
        {viewOnly ? (
          <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-900">
            This account is view-only. Assign INBOUND.PLAN for planning,
            INBOUND.APPROVE for release/close, or INBOUND.CANCEL for
            cancellation.
          </p>
        ) : null}
      </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_2fr_auto]">
            ...
            >
              <label className="relative flex-1">
                <span className="sr-only">Search Inbound Orders</span>
                <Search className="pointer-events-none absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-slate-400" />
                <input
                  key={filters.search}
                  name="search"
                  defaultValue={filters.search}
                  placeholder="Order ID, reference, or supplier"
          ...
          <p className="font-semibold text-slate-700">
            {!owner || !warehouse
              ...
              : orders.isPending
                ...
                : `${(orders.data?.total_items ?? 0).toLocaleString()} Inbound Order${orders.data?.total_items === 1 ? "" : "s"}`}
          </p>
        ...
        {!owner || !warehouse ? (
          <div className="px-5 py-16 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Select a warehouse and one of its served owners.
            </p>
          </div>
        ) : orders.isPending ? (
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <p className="font-semibold">
                    Inbound Orders could not be loaded
                  </p>
          ...
          <div className="px-5 py-16 text-center">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Create a draft from an approved Purchase Order or adjust the
              filters.
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full border-collapse text-left">
                ...
                <tbody className="divide-y divide-slate-200">
                  {orders.data.items.map((order) => (
                    <tr key={order.inbound_id} className="hover:bg-slate-50/70">
                      ...
                      <td className="px-5 py-4 text-right">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setSelectedId(order.inbound_id)}
                        >
                          {viewOnly ? (
            ...
            <ul className="divide-y divide-slate-200 md:hidden">
              {orders.data.items.map((order) => (
                <li key={order.inbound_id} className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-2">
                    <p className="text-xs text-slate-500">
                      {order.expected_arrival_at
                        ? `Expected ${dateFormatter.format(new Date(order.expected_arrival_at))}`
                        : "Arrival not set"}
                    </p>
                    <Button
                      variant="ghost"
